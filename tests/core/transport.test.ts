...
describe("createTransport", () => {
  it("builds absolute URLs when baseUrl is absolute", () => {
    ...
    expect(transport.buildUrl("/api/posts", { limit: 10 })).toBe(
      "https://api.fake.media/api/posts?limit=10",
    );
  });
  ...
  it("serializes JSON payloads and sends the Authorization header by default", async () => {
    ...
    const value = await transport.post("/api/auth/login", {
      username: "bob",
      password: "secret",
    });
  ...
  it("throws FakeMediaApiError with payload on non-OK response", async () => {
    const transport = createTransport({
      fetchImpl: (async () =>
        jsonResponse({ error: "Bad request" }, 400)) as unknown as typeof fetch,
    });
